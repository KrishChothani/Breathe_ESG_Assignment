...
```
┌─ DECISION D13: AI Chatbot Architecture ──────────────────────────┐
│ Chose:     LangGraph StateGraph for Text-to-SQL agent            │
│ Over:      Basic LLM prompt chain or naive RAG pipeline          │
│ Because:   Emissions querying requires precise aggregations      │
│            (e.g. "Total Scope 1 by plant"); Text-to-SQL achieves │
│            this, while the graph allows SQL validation + retries.│
│ Would ask PM: Do users need the ability to export the SQL        │
│            generated by the bot for their own BI tools?          │
│ Breaks if: LLM persistently hallucinates columns despite retries,│
│            falling back to "I cannot answer this".               │
└──────────────────────────────────────────────────────────────────┘
```
